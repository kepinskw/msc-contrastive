...
def get_model(args):
    ...
    print(f"Utworzono model dla metody Triplet z architekturą bazową: {args.arch}")
    return model
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, epoch, args,miner):
    ...
    for step, (images,labels) in enumerate(dataloader):#for step, batch_data in enumerate(dataloader):
        ...
        embeddings = model.forward_one(images_batch)
        
        indices_tuple = miner(embeddings, labels_batch)

        loss = criterion(embeddings, labels_batch, indices_tuple)

        if indices_tuple is not None and len(indices_tuple) > 0:
            loss = criterion(embeddings, labels_batch, indices_tuple)
            ...
            loss = torch.tensor(0.0) # Brak par znalezionych przez miner
...
def main(args):
    ...
    criterion = losses.ContrastiveLoss(pos_margin=0, neg_margin=args.margin).to(device)
    optimizer = get_optimizer(model, args)
    scheduler = get_scheduler(optimizer, args)

    miner = miners.PairMarginMiner(pos_margin=0, neg_margin=args.margin).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Trening
        avg_train_loss = train_one_epoch(model, train_loader, criterion, optimizer, device, epoch, args,miner)
