...
def parse_arguments():
    ...
    parser.add_argument('--optimizer', type=str, default='adam', choices=['adam', 'sgd'],
                        help='Optymalizator dla klasyfikatora liniowgtiego.')
    parser.add_argument('--lr', type=float, default=0.001, # Często wyższy LR dla SGD w linear probing
                        help='Szybkość uczenia dla klasyfikatora liniowego.')
    parser.add_argument('--weight_decay', type=float, default=0.0, # Zwykle brak lub mały weight decay dla głowy liniowej
    ...
    parser.add_argument('--lr_decay_epochs', type=str, default='60,80', help='Epoki do zmniejszenia LR dla StepLR (oddzielone przecinkami)')
...
def get_eval_dataloader(args):
    ...
    print(f"Załadowano dane treningowe i testowe dla ewaluacji: {args.dataset}")
...
def get_eval_optimizer(linear_classifier, args):
    ...
    if args.optimizer.lower() == 'adam':
        ...
        raise ValueError(f"Nieznany optymalizator ewaluacyjny: {args.optimizer}")
...
def get_eval_scheduler(optimizer, args):
    ...
    if args.scheduler == 'step':
        try:
            ...
            scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=decay_epochs, gamma=args.lr_decay_rate)
            print(f"Używany harmonogram ewaluacyjny: MultiStepLR (epoki: {decay_epochs}, gamma: {args.lr_decay_rate})")
        except ValueError:
             print(f"Błąd parsowania eval_lr_decay_epochs: {args.lr_decay_epochs}. Używam StepLR.")
        ...
        raise ValueError(f"Nieznany harmonogram ewaluacyjny: {args.scheduler}")
    return scheduler
...
def main(args):
    ...
    print(f"Rozpoczynanie treningu klasyfikatora liniowego na {args.epochs} epok...")
    ...
    for epoch in range(args.epochs):
        ...
        for step, (images, targets) in enumerate(train_loader):
            ...
            if (step + 1) % args.log_interval == 0:
                ...
                print(f'Eval Epoch [{epoch+1}/{args.epochs}] | Step [{step+1}/{len(train_loader)}] | '
                      f'Loss: {loss.item():.4f} | LR: {current_lr:.6f}')

        # Koniec epoki treningowej
        avg_epoch_loss = total_loss / total_samples
        epoch_accuracy = 100 * total_correct / total_samples
        print(f'--- Koniec Epoki Treningowej [{epoch+1}/{args.epochs}] ---')
